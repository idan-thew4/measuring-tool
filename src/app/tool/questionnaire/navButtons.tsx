...
export function NavButtons({ currentStep }: { currentStep: string[] }) {
  ...
  useEffect(() => {
    ...
    } else if (stepIdx < structure.content.length - 1) {
      next = `/tool/${structure.content[stepIdx + 1]["step-slug"]}/1/1`;
    }
    ...
    if (subChoiceIdx > 0) {
      ...
      const prevStep = structure.content[stepIdx - 1];
